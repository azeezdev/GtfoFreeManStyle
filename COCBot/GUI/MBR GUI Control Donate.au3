...
Func IsInGame()
	...
	While _CheckPixel($aIsGoldBar, True) = False ; Wait for MainScreen
		...
		If $iCount > 25 Then
			...
			ReturnAtHome()
			ForceCaptureRegion()
			If _CheckPixel($aChatTab, $g_bCapturePixel) = False Then
...
;~ 				If $g_iDebugSetlog  = 1 Then SetLog("$g_aiDonatePixel : (" & $g_aiDonatePixel [0] & "," & $g_aiDonatePixel [1] & ") $yPos: " & $yPos, $COLOR_DEBUG)
				...
				If ($g_bSkipDonTroops  = True And $g_bSkipDonSpells  = True)  Then
					$g_iActiveDonate = True
				...
				if $GtfoSpellType <> -1 and $g_bSkipDonSpells  = False then
					GtfoDonateTroopType($GtfoSpellType, 1, False, True,False)
				EndIf
				If $GtfoTroopType <> -1 and $g_bSkipDonTroops  = False Then
...
;~ 0xE4A438
	If WaitforPixel(28, 505 + $g_iBottomOffsetY, 30, 507 + $g_iBottomOffsetY, Hex(0xF0B549, 6), 5, 10) Then
		If $g_iDebugSetlogTrain = 1 Then SetLog("Click $aArmyTrainButton", $COLOR_DEBUG)
	...
	If WaitforPixel(815, 120 , 820, 125 , Hex(0xFD797D, 6), 10, 10) Then
		If $g_iDebugSetlogTrain = 1 Then SetLog("Wait for ArmyOverView Window", $COLOR_DEBUG)
		_Sleep(500)
		$getArmyCampCap = getOcrAndCapture("coc-ms", 110, 166, 82, 16, True)
		If $g_iDebugSetlogTrain = 1 Then SetLog("OCR $getArmyCampCap = " & $getArmyCampCap, $COLOR_DEBUG)
		...
		If $g_iDebugSetlogTrain = 1 Then SetLog("OCR $getSpellCap = " & $getSpellCap, $COLOR_DEBUG)
   ...
   _Sleep(250)
	...
	If WaitforPixel(230, 140 , 240, 145 , Hex(0xE8ECE0, 6), 10, 10) Then
		...
		If $g_iDebugSetlogTrain = 1 Then SetLog("Wait for Troops Window", $COLOR_GREEN)
		_Sleep(500)
		Local $getArmyCampCap = getOcrAndCapture("coc-train-quant1", 45, 160, 70, 18, True)
		If $g_iDebugSetlogTrain = 1 Then SetLog("OCR $sArmyInfo = " & StringLeft($getArmyCampCap, StringLen($getArmyCampCap)-3), $COLOR_DEBUG)
...
;~ 			DonatedTroop($GtfoTroopType, $iTroopTrain)
	...
	If WaitforPixel(430, 140 , 440, 145 , Hex(0xE8ECE0, 6), 10, 10) Then
		If $g_iDebugSetlogTrain = 1 Then SetLog("Wait for Spells Window", $COLOR_DEBUG)
		_Sleep(500)
		Local $iSplAdj = 1
		if $TotalSpell >= 10 then $iSplAdj = 2
		$getSpellCap = getOcrAndCapture("coc-train-quant1", 48, 160, 40, 18, True)
		If $g_iDebugSetlogTrain = 1 Then SetLog("OCR $sArmyInfo = " & StringLeft($getSpellCap, StringLen($getSpellCap)-$iSplAdj), $COLOR_DEBUG)
...
;~ 		$iDarkTotal -= $tempDElixirSpent
	...
	iF $ClanMode = True Then Return True
...
Func iDonateWindow($Open = True)
	...
	ForceCaptureRegion()
	Local $DonatePixelCheck = _MultiPixelSearch($iLeft, $iTop, $iRight, $iBottom, 50, 1, Hex(0x98D057, 6), $aChatDonateBtnColors, 20)
...
Func GtfoDonateTroopType($Type, $Quant = 0, $Custom = False, $g_iActiveDonateAll = False,$isTrool=True)
	If $g_iDebugSetlog  = 1 Then SetLog("$DonateTroopType Start: " & NameOfTroop($Type), $COLOR_DEBUG) ;Debug
	...
	If $g_iDebugOCRdonate  = 1 Then $debugOcr = $oldDebugOcr
...
Func GtfoDetectSlotTroop($Type)
	...
	If $Type >= $eBarb And $Type <= $eBowl Then
		For $Slot = 0 To 5
			...
			If $g_iDebugSetlog = 1 Then Setlog("Slot: " & $Slot & " SearchImgloc returned >>" & $FullTemp[0] & "<<", $COLOR_DEBUG)
			...
			If $FullTemp[0] <> "" Then
				For $i = $eBarb To $eBowl
					...
					;If $g_iDebugSetlog = 1 Then Setlog(NameOfTroop($i) & " = " & $sTmp, $COLOR_DEBUG)
					If StringInStr($FullTemp[0] & " ", $sTmp) > 0 Then
						If $g_iDebugSetlog = 1 Then Setlog("Detected " & NameOfTroop($i), $COLOR_DEBUG)
